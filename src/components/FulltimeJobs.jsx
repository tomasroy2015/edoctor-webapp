...
import jobFulltimeService from '../service/FulltimeJobService';
...
class FulltimeJobs extends Component {
    ...
    componentWillMount() {
        jobFulltimeService.getFulltimeJobList()
        .then(res => {
            console.log(res);
    ...
    render() {
        const columns = [{
            ...
            formatter: (cellContent, row) => {
                return (
                    <span>{row.work_address1} {row.work_address2} {row.work_address3}</span>
                );            
            }
          },
          {
...
export default withAlert(FulltimeJobs);
