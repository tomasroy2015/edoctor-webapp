...
class Doctors extends Component {
    ...
    componentWillMount() {
        ...
        .then(res => {
            console.log(res);
            this.setState({doctorList:res.content});
        })
        .catch(err => this.props.alert.error("Error" + err));
    }
    ...
    render() {
        const columns = [{
            ...
            formatter: (cellContent, row) => {
                return (
                    <span>{row.address1} {row.address2} {row.address3}</span>
                );            
            }
          }, {
            dataField: 'tel',
            ...
            formatter: (cellContent, row) => {
                return (
                    <FontAwesomeIcon icon={faEdit} className="edit-button"  onClick={this.handleShow}/>
                );            
            }
          }];
                ...
                <Grid>
                    <Row className="show-grid text-left" style={{marginTop:'6%'}}>
                        ...
                        <Col xs={12} md={12} sm={8} >
                          <Button bsStyle="success" type="button" onClick={this.addUser}>Add new</Button>
                        </Col>
                        
                        <Col xs={12} md={12} sm={8} style={{marginTop:'2%'}}>
                          {this.state.doctorList &&
                           <ReactTable keyField='job_code' data={ this.state.doctorList } 
                                columns={ columns } 
